...
export const AnalyticsScreen: React.FC = () => {
  ...
  const data = stats;
  const usageTrend = data?.usageTrend7d || [];
  const masteryBySubject = data?.masteryBySubject || [];
  const stageMix = data?.stageDistribution || [];
  const topSkills = data?.skillMovement?.top || [];
  const weakSkills = data?.skillMovement?.weak || [];
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.kpiRow}>
        <View style={styles.kpiCard}>
          ...
          <Text style={styles.kpiValue}>{data?.activeParents7d ?? '—'}</Text>
          <Text style={styles.kpiSub}>Last 7 days</Text>
        </View>
        <View style={styles.kpiCard}>
          <Text style={styles.kpiLabel}>Active children</Text>
          <Text style={styles.kpiValue}>{data?.activeChildren7d ?? '—'}</Text>
          <Text style={styles.kpiSub}>Last 7 days</Text>
        </View>
        <View style={styles.kpiCard}>
          <Text style={styles.kpiLabel}>Avg session</Text>
          <Text style={styles.kpiValue}>
            {typeof data?.avgSessionMin === 'number' ? `${data.avgSessionMin.toFixed(1)} min` : '—'}
          </Text>
          <Text style={styles.kpiSub}>Median</Text>
        </View>
        <View style={styles.kpiCard}>
          <Text style={styles.kpiLabel}>Retention D7</Text>
          <Text style={styles.kpiValue}>{typeof data?.retentionD7 === 'number' ? `${data.retentionD7}%` : '—'}</Text>
      ...
      <View style={styles.gridRow}>
        ...
        <View style={[styles.card, styles.flex1]}>
          ...
          {masteryBySubject.length === 0 ? (
            <Text style={styles.emptyText}>No mastery data yet.</Text>
          ) : (
      ...
      <View style={styles.gridRow}>
        <View style={[styles.card, styles.flex1]}>
          ...
          {stageMix.length === 0 ? (
            <Text style={styles.emptyText}>No stage data yet.</Text>
          ) : (
            stageMix.map((seg) => (
        ...
        <View style={[styles.card, styles.flex1]}>
          ...
          {topSkills.length === 0 && weakSkills.length === 0 ? (
            <Text style={styles.emptyText}>No skill movement yet.</Text>
          ) : (
            <>
...
const styles = StyleSheet.create({
  ...
  emptyText: { color: 'rgba(255,255,255,0.55)', fontSize: 11, marginTop: 12 },
