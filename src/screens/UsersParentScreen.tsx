...
export const UsersParentScreen: React.FC = () => {
  ...
  const fetchPage = React.useCallback(async (pageIndex: number) => {
    ...
    parentSnap.forEach((docSnap) => {
      ...
      const name = [data.firstName, data.lastName].filter(Boolean).join(' ') || displayName || data.email || 'Unknown';
      const lastActiveAt =
    ...
    const hydrated = rows.map((parent) => ({
      ...parent,
      childrenCount: childCounts[parent.id] || 0,
    }));

    setParents(hydrated);
    setHasNext(parentSnap.size === PAGE_SIZE);
    if (parentSnap.size > 0) {
      const lastDoc = parentSnap.docs[parentSnap.docs.length - 1];
      ...
      <View style={styles.card}>
        ...
        {loading ? (
          ...
          <View style={styles.emptyRow}>
            <Text style={styles.emptyText}>No parent profiles found.</Text>
          </View>
        ) : (
          filteredParents.map((parent) => (
            <View key={parent.id} style={styles.tableRow}>
              <Text style={styles.tableCell}>{parent.name}</Text>
              <Text style={styles.tableCell}>{parent.childrenCount}</Text>
              <Text style={styles.tableCell}>{getStatus(parent.lastActiveAt)}</Text>
              <Text style={styles.tableCell}>{formatLastActive(parent.lastActiveAt)}</Text>
              <View style={styles.actionCellWrap}>
                <Pressable
                  accessibilityRole="button"
                  onPress={() => navigate('parentDetail', { id: parent.id })}
                ...
                <Pressable
                  ...
                  onPress={async () => {
                    ...
                    await updateDoc(ref, {
                      ...
                      blockedAt: parent.isBlocked ? null : serverTimestamp(),
                    });
                  }}
                  style={({ pressed }) => [styles.actionButton, pressed && styles.actionButtonPressed]}
                >
      ...
      <Modal visible={filterOpen} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            <Text style={styles.modalTitle}>Filter Parents</Text>
